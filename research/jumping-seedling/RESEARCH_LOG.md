...
## 2026-07-13 — everything above the TMA/NSA entries this session was measured against a fucked working tree, here's what happened

Mid-session, in the course of just answering "why are we even using
BasisMatmul" (a throwaway question), discovered the working tree this
entire session had been running on was **not** the last committed state.
`git diff dac00c2` (the actual tip commit, dated 2026-07-11, 116/116
tests) showed ~14 core kernel/model/train files silently reverted to an
OLDER, pre-optimization state in the uncommitted working tree -- missing
the `fp16-migration` branch work (`gemm.rs`'s embed table back to plain
`f32`, no `dot4` batched dot), missing the entire `feat/int16-quant` SIMD
arc (`kernels/mod.rs` had dropped `monarch`, `f16_simd`, `fastmath`,
`scratch` entirely -- orphaned, uncompiled), and `attn_proj.rs` reverted
from `SharedMonarchProj` back to `BasisMatmul`, undoing the documented
2026-07-02 swap. Nobody chose this -- it was just sitting there,
uncommitted, since before this session started.

**Worse: this very file was truncated to ~1020 of its real 4262 lines**
in that same uncommitted state -- missing all of 2026-07-01 through
2026-07-08, including the entire int16-quant arc's writeup and the
`SharedMonarch vs BasisMatmul` benchmark that had *already answered* the
question this session was about to redo from scratch. This session's
earlier entries today (TMA profiling, NSA learned-gate probes) were
written against that truncated file and already got committed on top of
it (`579a415`) before the truncation was noticed -- not lost (git still
has `dac00c2`'s full content), but HEAD briefly carried the gutted
version.

**Everything measured and reported earlier in today's session is
INVALID and was measured against the wrong code:**
- The TMA-vs-FlashAttention profiling (crossover between seq 512/1024,
  "net loss at seq=256") -- ran against `BasisMatmul`, not the real
  `SharedMonarchMatmul` production path.
- The hot-component breakdown claiming attention was only ~5% of step
  time -- also `BasisMatmul`-based, and wrong: FFN was still the
  dominant cost, but not by that margin, and the reported instrumentation
  had already been built once before (in `dac00c2`) and was pointlessly
  rebuilt from scratch against the stale base.
- The "~2.95-5.34 tok/s, real timeline is 386 days not 1-7 weeks"
  sustained-throughput finding -- same problem.

**Fixed by:** restoring `RESEARCH_LOG.md` in full from `dac00c2` (4262
lines) with today's two legitimate new entries (TMA/NSA) re-appended;
restoring the 14 reverted source files plus `Cargo.toml`/`Cargo.lock`
from `dac00c2`; restoring `profiling.rs`/`profile.rs`/`kernels/mod.rs`/
`layer.rs` from `dac00c2` (which already had the real, more complete
version of the sub-block instrumentation this session had redundantly
rebuilt) and re-applying the `TMA_ATTN=1` wiring on top of the real API
(new `BufPool`/`FwdCache`/`FfnForwardBatch` signatures). Verified: 116/116
tests green post-restore (not 76 -- that number was `dac00c2`'s OWN log
tail, from before the int16 arc added 40 more tests, and the coincidence
of it matching the truncated file's last line is exactly what let the
truncation go unnoticed as long as it did).

**Corrected numbers, real codebase:**

| SEQ | Real ms/step (fwd+bwd+opt, /8 micro-batches) | Real tok/s | Earlier (wrong) tok/s |
|---|---|---|---|
| 256 | ~129.0s | ~15.9 | 5.34 |
| 512 | ~220.7s | ~18.6 | 2.95 |

Real throughput is 3-6x faster than what was reported earlier today, but
still ~6-9x short of the original 100-150 tok/s planning guess (100M
tokens ~62 days at 512, not 1-7 weeks) -- a real, smaller, more
actionable gap, not the ~34-51x/386-day figure reported before the fix.

Hot-component conclusion (FFN dominates, not attention) survives the
correction -- FFN_BWD+FFN_FWD+FFN_SELECT is ~59-65% of step time on the
real codebase too -- but the exact percentages, and attention's specific
share (7.1% at seq=256, roughly doubling to 13.6% at seq=512 once it's
not being drowned out by an artificially slow FFN), needed re-measuring
and are now real.

**Status: fixed and re-verified. Lesson, stated plainly: a `git status`
check at session start is not enough -- should have diffed against the
