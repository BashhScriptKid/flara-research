"""Robust centroid construction (Fable's #2 mitigation, tested after
floor-read failed cleanly): replaces the arithmetic-mean k-means
centroid with a geometric median, computed via a few Weiszfeld
iterations after the standard k-means assignment settles.

Why geometric median specifically: the adversarial mass-heavy-decoy
test found genuine CENTROID CAPTURE -- a single high-magnitude point
dragging an arithmetic-mean centroid away from the true content
(83.33% failure rate). The arithmetic mean has a 0% breakdown point
(a single unbounded point can move it arbitrarily far); the geometric
median has up to a 50% breakdown point (up to half the points in a
cluster can be adversarial before it's dragged arbitrarily far) --
this attacks the actual capture mechanism, unlike floor-read (tested
separately, failed identically for use_floor=True/False) which tried
to add a fallback built from the same fragile (arithmetic-mean-style,
flat-average) construction that has no signal to contribute regardless
of routing outcome.

Weiszfeld's algorithm: iteratively reweighted mean, weight_i =
1 / ||x_i - y_t||, run for a few iterations starting from the
arithmetic mean. Implemented in the same dense/vectorized onehot style
as the existing k-means (weight[i,c] = onehot[i,c] / dist(k_block[i],
centroid[c]), replacing the uniform member weight of 1 used for the
arithmetic mean).
"""
...
def _kmeans_buckets_robust(k_block: Tensor, n_buckets: int, iters: int, sm_scale: float, weiszfeld_iters: int = 5, eps: float = 1e-8):
    Bl = k_block.shape[-2]
    ...
    assign = None
    # standard k-means assignment (arithmetic-mean update, cheap, settles the partition)
    ...
    # robust refinement: Weiszfeld iterations on the FIXED final assignment,
    # replacing the arithmetic mean with a geometric-median-seeking update.
    onehot = F.one_hot(assign, n_buckets).to(k_block.dtype)  # (...,Bl,n_buckets)
    counts = onehot.sum(dim=-2).unsqueeze(-1)
    for _ in range(weiszfeld_iters):
        # dist[i,c] = ||k_block[i] - centroids[c]||, but only meaningful for i in bucket c
        dist = (k_block.unsqueeze(-2) - centroids.unsqueeze(-3)).norm(dim=-1)  # (...,Bl,n_buckets)
        inv_dist = onehot / (dist + eps)  # zero outside membership, 1/dist inside
        weighted_sums = torch.einsum("...bc,...bd->...cd", inv_dist, k_block)
        weight_totals = inv_dist.sum(dim=-2).unsqueeze(-1)
        new_centroids = weighted_sums / weight_totals.clamp(min=eps)
        centroids = torch.where(counts == 0, centroids, new_centroids)

    return centroids, assign


def monarch_meta_bucket_route_robust(
    q: Tensor,
    k: Tensor,
    v: Tensor,
    B: int,
    ...
    for l in range(L):
        ...
        centroids, assign = _kmeans_buckets_robust(k_l, n_buckets, kmeans_iters, sm_scale, weiszfeld_iters)
