...
public class BenchmarkAnalyzer
{
    ...
    public async Task RunObfuscationAnalysis(List<string> models, int maxSamples = 50)
    {
        var sep = new string('=', 60);
        Console.WriteLine(sep);
        Console.WriteLine("OBFUSCATION DETECTION ANALYSIS");
        Console.WriteLine(sep);

        var obfFile = "benchmark_data/obf_trigger.json";
        var benignFile = "benchmark_data/obf_benign.json";

        if (!File.Exists(obfFile) || !File.Exists(benignFile))
        {
            Console.WriteLine("\nObfuscation data not found. Run download_benchmarks.py first.");
            return;
        }

        var obfSamples = JsonSerializer.Deserialize<List<string>>(File.ReadAllText(obfFile));
        var benignSamples = JsonSerializer.Deserialize<List<string>>(File.ReadAllText(benignFile));

        if (obfSamples.Count > maxSamples)
            obfSamples = obfSamples.GetRange(0, maxSamples);
        if (benignSamples.Count > maxSamples)
            benignSamples = benignSamples.GetRange(0, maxSamples);

        Console.WriteLine($"\nSamples: {obfSamples.Count} obfuscation + {benignSamples.Count} benign");
        ...
        foreach (var model in models)
        {
            ...
            for (int i = 0; i < obfSamples.Count; i++)
            {
                var r = await AnalyzeSampleWithModel(analyzer, obfSamples[i], "obfuscation", "obfuscation", model);
                results.Add(r);
                if ((i + 1) % 10 == 0) Console.WriteLine($"  {i + 1}/{obfSamples.Count} obfuscation");
            }

            for (int i = 0; i < benignSamples.Count; i++)
            {
                var r = await AnalyzeSampleWithModel(analyzer, benignSamples[i], "benign", "benign", model);
                results.Add(r);
                if ((i + 1) % 10 == 0) Console.WriteLine($"  {i + 1}/{benignSamples.Count} benign");
            }

            allModelResults[model] = results;

            var csvPath = $"benchmark_data/obf_results_{SanitizeModelName(model)}.csv";
            SaveCsv(results, csvPath);
        }

        SaveCombinedCsv(allModelResults, "benchmark_data/obf_combined_results.csv");
        Console.WriteLine($"\nSaved to benchmark_data/obf_combined_results.csv");

        // Print comparison
        Console.WriteLine("\n" + sep);
        Console.WriteLine("OBFUSCATION vs BENIGN");
        Console.WriteLine(sep);
        Console.WriteLine($"{"Model",-45} {"Obf D",10} {"Ben D",10} {"Gap",10} {"F1",8}");
        Console.WriteLine(new string('-', 93));

        foreach (var model in models)
        {
            var results = allModelResults[model];
            var obfR = results.FindAll(r => r.Label == "obfuscation");
            var benR = results.FindAll(r => r.Label == "benign");

            if (obfR.Count == 0 || benR.Count == 0) continue;

            var oDeltas = obfR.ConvertAll(r => r.Delta);
            var bDeltas = benR.ConvertAll(r => r.Delta);
            float oMean = Mean(oDeltas);
            float bMean = Mean(bDeltas);
            float gap = MathF.Abs(bMean - oMean);
            ...
            {
                var tp = obfR.Count(r => r.Delta >= threshold);
                var fp = benR.Count(r => r.Delta >= threshold);
                var fn = obfR.Count(r => r.Delta < threshold);
            ...
            Console.WriteLine($"{shortName,-45} {oMean,10:F4} {bMean,10:F4} {gap,10:F4} {bestF1,8:F4}");
        }
    }
